...
//    stat?c=problem_statement&pm=13506&rd=16080
...
function hasPath(board) {
  ...
  function hasPathHelper(board, i, j) {
    if (i < 0 || i > 1 || j < 0 || j >= board[0].length
    ...
    return hasPathHelper(board, i + 1, j) ||
    hasPathHelper(board, i, j + 1) || hasPathHelper(board, i - 1, j);
  }
  return hasPathHelper(board.slice(), 0, 0) ||
  hasPathHelper(board.slice(), 1, 0);
}
